...
function LandingPage(props) {
  ...
  useEffect(() => {
    Axios.get(`${BACK_SERVER_URL}/api/book/getbooks`).then(response => {
    ...
    Axios.get(`${BACK_SERVER_URL}/api/comment/getCommentscount`).then(response => {
      if (response.data.success) {
  ...
  return (
    <div>
      <div style={{height:"600px",backgroundImage: `url('./booksearch.jpg')`, backgroundSize:"100% 600px", paddingTop:"100px"}}>
      <center style={{marginTop:"50px"}}>
      <h1>국내에서 다양한 책과 리뷰를 만나보세요 </h1>
      <br/><br/>
      <Search
