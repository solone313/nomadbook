...
router.get("/getsidebooks", (req, res) => {
  ...
  Book.find()
    ...
    .exec((err, books) => {
      if (err) return res.status(400).send(err);
      shuffle(books);
      res.status(200).json({ success: true, books: books.slice(0, 6) });
    });
});
...
router.post("/searchresult", (req,res) => {
  Book.find({$or :[{title : { $regex: req.body.value }},{ author : { $regex: req.body.value }},{publisher : { $regex: req.body.value }}]}).exec(
    (err, searchresult) => {
      if (err) return res.status(400).send(err);
